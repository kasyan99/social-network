...
const TOGGLE_IS_FOLLOWING_PROGRESS =
  "social-network/users/TOGGLE-IS-FOLLOWING-PROGRESS"
    ...
    case FOLLOW:
      return {
        ...
        usersList: state.usersList.map((user) =>
          user._id === action.id ? { ...user, followed: true } : user
        ),
      }
    case UNFOLLOW:
      return {
        ...state,
        usersList: state.usersList.map((user) =>
          user._id === action.id ? { ...user, followed: false } : user
        ),
      }
    case SET_USERS:
      return {
...
export const usersActions = {
  setUsers: (users: Array<ProfileType>) =>
    ({ type: SET_USERS, users } as const),

  Followt: (userId: string) => ({ type: FOLLOW, id: userId } as const),

  Unfollow: (userId: string) => ({ type: UNFOLLOW, id: userId } as const),
  ...
  ToggleIsFollowing: (isFetching: boolean, userId: string) =>
...
export const followToggleThunkCreator =
  (user: ProfileType): ThunkType =>
  (dispatch) => {
    dispatch(usersActions.ToggleIsFollowing(true, user._id))
    user.followed
      ? dispatch(unfollowThunkCreator(user._id))
      : dispatch(followTnunkCreator(user._id))
  }
